...
class ChatBase:
    ...
    # getattr(). Special-case drivers whose protocol is not OpenAI-compatible
    # (e.g. Anthropic Messages API) set this explicitly in their __init__.
    # OpenAI-compatible drivers leave it empty: ChatBase auto-wires the generic
    # reasoning stream from config — see ``_ensure_openai_compat_reasoning_stream``.
    _native_stream_provider: str = ''
    # Raw ``openai`` SDK client used by the generic reasoning handler. Built lazily
    # by ChatBase from the driver's ``_llm``; never set per-provider.
    ...
    def _ensure_openai_compat_reasoning_stream(self) -> None:
        ...
        Most providers (DeepSeek, GMI Cloud, Qwen, xAI, Ollama, …) talk the OpenAI
        Chat Completions protocol via ``ChatOpenAI`` and emit reasoning on
