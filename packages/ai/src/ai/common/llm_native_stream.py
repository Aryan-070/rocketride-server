...
Special-case drivers opt in by setting ``self._native_stream_provider`` to a
registered key (e.g. ``\"anthropic\"``). For OpenAI-compatible drivers ChatBase
auto-wires the ``\"openai_compat_reasoning\"`` key from ``capabilities.reasoning``
— no per-provider code. :meth:`ChatBase.chat_string` calls
